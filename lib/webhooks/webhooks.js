...
function validateRequest(authToken, twilioHeader, url, params) {
  ...
  if (!isValid) {
    const searchParams = new URLSearchParams(urlObject.query)
    const hasProjectId = searchParams.has('project_id')
    const hasCallId = searchParams.has('call_id')
    const hasMessageId = searchParams.has('message_id')
    const { project_id, call_id, call, message_id } = params || {}
    if (hasProjectId || hasCallId || hasMessageId || project_id || call_id || message_id || call?.project_id) {
      console.warn('It seems that you are trying to validate a RELAY request using the Compatibility SDK "validateRequest" method. Please use method XXX from package YYY to validate all types of SignalWire requests (RELAY and Compatibility).')
    }
  }
